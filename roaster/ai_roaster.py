"""AI-powered brutal roast generator using OpenAI API."""
...
def generate_ai_roasts(profile_text: str, num_roasts: int = 3) -> list[str]:
    ...
    """
    if not _HAS_OPENAI:
        print("[AI Roaster] ERROR: openai module not installed")
        return []

    api_key = os.getenv("OPENAI_API_KEY")
    if not api_key:
        print("[AI Roaster] ERROR: OPENAI_API_KEY not found in environment")
        print("[AI Roaster] Available env vars:", list(os.environ.keys()))
        return []

    if not api_key.startswith("sk-"):
        print(
            f"[AI Roaster] ERROR: OPENAI_API_KEY appears invalid (should start with sk-, got {api_key[:20]}...)"
        )
        return []

    print(f"[AI Roaster] API Key loaded: {api_key[:20]}...")

    try:
        print("[AI Roaster] Creating OpenAI client...")
...
Generate exactly {num_roasts} roasts, one per line. JUST the roasts, no intro. Be BRUTAL."""
        ...
        print(f"[AI Roaster] ✓ Generated {len(roasts)} brutal roasts")
...
def generate_ai_suggestions(profile_text: str) -> list[str]:
    ...
    """
    if not _HAS_OPENAI:
        print("[AI Suggestions] ERROR: openai module not installed")
        return []
    ...
    print(f"[AI Suggestions] API Key loaded: {api_key[:20]}...")

    try:
        print("[AI Suggestions] Creating OpenAI client...")
        client = openai.OpenAI(api_key=api_key)
